...
class ConstantStrings
{
    ...
    static [string] $policyDefinitionUrl = "https://raw.githubusercontent.com/AsrOneSdk/" + `
        "policy-based-replication/allowZone/policy-1.0/policy.json"
    static [string] $policyParametersUrl = "https://raw.githubusercontent.com/AsrOneSdk/" + `
        "policy-based-replication/allowZone/policy-1.0/parameters.json"
...
class Errors
{
    ...
    ### <param name="sourceRGName">Source resource group name.</param>
    ### <param name="vaultRGName">Vault resource group name.</param>
    ### <param name="targetRGName">Target resource group name.</param>
    ### <return>Error string.</return>
        ...
        [string] $targetRGName)
    {
        ...
        "`n2. Azure Powershell - Run the following cmdlets to manually add roles for each of " + `
        "resource groups: " + `
        "`nNew-AzRoleAssignment -ObjectId $principalId -ResourceGroupName $sourceRGName " + `
        "-RoleDefinitionName $roleName"
        "`nNew-AzRoleAssignment -ObjectId $principalId -ResourceGroupName $vaultRGName " + `
        "-RoleDefinitionName $roleName"
...
function Add-RoleAssignments()
{
    ...
    if ($null -ne $servicePrincipal)
    {
        ...
        {
            # Displaying workaround for manual role addition.
            $message =
                [Errors]::RoleAssignmentFailed(
                    $objectId,
                    "Owner",
                    $roleCreationCount,
...
function Log-AdditionalURLs()
{
    ...
    $urlOutput = "`nPolicy Compliance Page: " + [ConstantStrings]::portalPolicyCompliancePageLink + "`n"
    $urlOutput += "`nDetailed Policy Assignment Compliance Page: " + $assignmentCompliancePage + "`n"
    $urlOutput += "`nVault ResourceGroup Deployments: " + $vaultResourceGroupDeploymentUrl + "`n"
    $urlOutput += "`nTarget ResourceGroup Deployments: " + $targetResourceGroupDeploymentUrl + "`n"
    $urlOutput += "`nReplicated Items List: " + $replicatedItemsListUrl + "`n"
    $urlOutput += "`nSite Recovery Jobs: " + $replicationJobsUrl + "`n"
    $urlOutput += "`n"
...
$OutputLogger = [Logger]::new('PolicyBasedReplication-' + $StartTime, $logFileLocation)
$OutputLogger.Log(
    $MyInvocation,
    "StartTime - $StartTime",
    [LogType]::INFO)
